...
#
# Reads SPX.csv (or any uploaded CSV with a Date column), lets you select:
# - column (Close/Open/etc.)
# - timeframe (date range)
# - number of Fourier harmonics
# Fits an "identical wave formula" (Fourier series with trend)
# Then projects forward using the SAME fitted formula.
# Also computes:
# - Hurst exponent (R/S method + optional aggregation slope)
# - Mean reversion stats (AR(1) on returns, half-life, OU-ish interpretation)
#
# Requirements: streamlit, numpy, pandas, pillow (not needed), matplotlib (streamlit includes), python-dateutil (pandas deps)
#
import os
import io
import math
...
# Safety / defaults
# -----------------------------
st.set_page_config(page_title="SPX Wave Formula + Hurst", layout="wide")

DEFAULT_CSV_NAME = "SPX.csv"
MAX_POINTS_FOR_FIT = 25_000  # guardrail for speed (you can raise)
EPS = 1e-12
...
def parse_csv(file_bytes: bytes, filename: str = "data.csv") -> pd.DataFrame:
    ...
    # normalize column names (keep original but strip spaces)
    ...
    for c in df.columns:
        if c.lower() in ("date", "datetime", "timestamp", "time"):
            ...
            break
    if date_col is None:
        raise ValueError("Could not find a Date column (expected a column named Date).")
...
def get_default_csv_bytes() -> Optional[bytes]:
...
def make_time_index(n: int, dt: float = 1.0) -> np.ndarray:
    # t in days (or steps) starting at 0
...
def build_design_matrix(t: np.ndarray, T: float, H: int) -> np.ndarray:
    # columns: [1, t, cos(2πk t/T), sin(2πk t/T)]
    n = len(t)
...
def fit_wave_formula(y: np.ndarray, harmonics: int) -> WaveFitResult:
    ...
    # "Identical wave formula": choose base period as length of window (captures full-window cycles)
    T = float(max(2.0, n - 1))
...
def wave_formula_text(fit: WaveFitResult) -> str:
    ...
    # show denormalized formula for user:
    # y = (a0 + a1 t + Σ(b cos + c sin))*std + mean
    lines = []
    ...
    # truncate coefficient list if big
    show = min(H, 20)
...
def hurst_rs(series: np.ndarray, min_chunk: int = 16, max_chunk: int = 512) -> Tuple[float, pd.DataFrame]:
    """
    R/S method:
    For various chunk sizes n, compute mean R/S and fit log(R/S) ~ H log(n) + c.
    Returns H and details dataframe.
    """
    ...
    # Choose chunk sizes as powers of 2 within range
    sizes = []
    s = min_chunk
...
def mean_reversion_stats(price: np.ndarray) -> Dict[str, Any]:
    """
    Simple mean-reversion diagnostics:
    - AR(1) on log returns: r_t = c + phi r_{t-1} + e_t
    - If phi < 0: strong mean reversion in returns (oscillatory)
    - If 0 < phi < 1: positive autocorr in returns (momentum)
    - Also compute "half-life" for mean reversion of deviations using OU-ish
      by fitting AR(1) on demeaned log prices: x_t = a + b x_{t-1} + e
      half_life = -ln(2)/ln(b) for 0<b<1
    """
...
with st.sidebar:
    ...
    default_bytes = get_default_csv_bytes()
    source = st.radio("CSV source", ["Use SPX.csv in app folder", "Upload CSV"], index=0)

    csv_bytes = None
    filename = "SPX.csv"
    if source == "Use SPX.csv in app folder":
        if default_bytes is None:
            st.error("Could not find SPX.csv next to app.py. Upload a CSV instead.")
        else:
...
with cA:
    col = st.selectbox("Select column", cols, index=cols.index("Close") if "Close" in cols else 0)
with cB:
...
plt.plot(dates, y)
plt.title(f"{col} over selected timeframe")
plt.xlabel("Date")
plt.ylabel(col)
plt.tight_layout()
st.pyplot(fig)

# Fit button to avoid rerun compute surprises
st.divider()
...
# Evaluate fitted wave on the original window length (for plotting)
n_window = len(y)
...
# Build forward dates using business-day frequency if index looks daily
# We'll infer typical spacing:
if len(dates) >= 2:
...
plt.title(f"Wave formula fit (H={fit.harmonics}) and forward projection ({n_fwd} steps)")
plt.xlabel("Date")
plt.ylabel(col)
plt.legend()
plt.tight_layout()
...
# Choose whether to compute Hurst on price or returns (both useful)
...
# -----------------------------
# Download: fitted + forecast series
# -----------------------------
st.divider()
